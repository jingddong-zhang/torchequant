...
log_filename = 'train_log_equant_0922.txt'
...
while out_iters < 1:
    ...
    max_iters = 10000
    learning_rate = 0.005
    estimate_w = 2*math.pi/(300/9999*len(data))
    ...
    while i < max_iters:
        ...
        if i%100 == 0:
            ...
            # plt.subplot(132)
            # plt.plot(data.detach()[:, 0], data.detach()[:, 1])
            # plt.subplot(133)
    ...
    model.load_state_dict(torch.load('./data/model_mitotic_equant.pkl'))
    max_iters = 3000
    learning_rate = 0.001
    ...
    while i < max_iters:
        ...
        if i%100 == 0:
            print(i, "loss=", loss.item(), loss_bound.item(),loss_angle.item(),mod_equant.item(),loss_surface.item())
    ...
    torch.save(best_model, './data/model_mitotic_equant.pkl')
